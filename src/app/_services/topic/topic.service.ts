...
export class TopicService {
  ...
  public getTopics(query?: any): Observable<any> {
    return this.http.get(environment.apiUrl + '/api/topics?filter=' + JSON.stringify(query))
      .map(res => res || []);
  }
  ...
  public suggestionPerQuery(query: string): Observable<any> {
    return this.http.get(environment.searchUrl + '/api/search/topics/suggest?field=name&query=' + query)
      .map(res => res || []);
  }
