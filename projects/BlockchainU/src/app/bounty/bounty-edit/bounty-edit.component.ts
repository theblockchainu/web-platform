...
@Component({
    ...
    styleUrls: ['./bounty-edit.component.scss']
})

export class BountyEditComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    public sidebarFilePath = 'assets/menu/bounty-static-left-sidebar-menu.json';
    ...
    public bountyData: any;
    public isBountyActive = false;
    public activeBounty = '';
    ...
    bountyVideoPending: Boolean;
    bountyImage1Pending: Boolean;
    bountyImage2Pending: Boolean;
    ...
    constructor(
        ...
        this.activatedRoute.params.subscribe(params => {
            ...
            this.connectPaymentUrl = 'https://connect.stripe.com/express/oauth/authorize?response_type=code' +
                ...
                + '/console/account/payoutmethods&state=' + environment.clientUrl + '/bounty/' + this.bountyId
                + '/edit/' + this.step;
    ...
    public ngOnInit() {
        console.log('Inside oninit bounty');
        this.setTags();
    ...
    private setTags() {
        this.titleService.setTitle('Create Bounty');
    ...
    private initializeCalendarCheck(bountyData: any) {
    ...
    public getContents(contents) {
        ...
        if (this.sidebarMenuItems) {
            ...
            // this.sidebarMenuItems = this._leftSideBarService.updateBountySideMenu(this.bounty.value, this.sidebarMenuItems);
            // this.sidebarMenuItems[2]['submenu'] = [];
        }
    ...
    private initializeFormFields() {
        ...
        this.languagePickerService.getLanguages()
            .subscribe((languages) => {
                ...
                this.filteredLanguageOptions = this.bounty.controls.selectedLanguage.valueChanges
                    .pipe(startWith(null),
        ...
        this.bountyVideoPending = true;
        this.bountyImage1Pending = true;

        // this.bounty.controls['academicGyan'].valueChanges.subscribe(res => {
        // 	this.validateGyan();
        // });
    ...
    private initializeBounty() {
        console.log('Inside init bounty');
        if (this.bountyId) {
            this._collectionService.getCollectionDetail(this.bountyId, this.query)
                .subscribe((res: any) => {
                    console.log(res);
                    this.bountyData = res;
                    if (this.bountyData.payoutrules && this.bountyData.payoutrules.length > 0) {
                        this.payoutRuleNodeId = this.bountyData.payoutrules[0].id;
                        this.payoutRuleAccountId = this.bountyData.payoutrules[0].payoutId1;
                    }
    ...
    private initializeFormValues(res) {
        ...
        if (res.language && res.language.length > 0) {
            ...
            this.bounty.controls.selectedLanguage.patchValue(res.language[0]);
        }
        // aboutHost TBD
        this.bounty.controls.aboutHost.patchValue(res.aboutHost);

        // Title
        this.bounty.controls.title.patchValue(res.title);

        // Headline
        this.bounty.controls.headline.patchValue(res.headline);

        // Description
        this.bounty.controls.description.patchValue(res.description);

        // Difficulty Level
        this.bounty.controls.difficultyLevel.patchValue(res.difficultyLevel);

        // Notes
        this.bounty.controls.notes.patchValue(res.notes);

        // Seats
        this.bounty.controls.maxSpots.patchValue(res.maxSpots);

        // Photos and Videos
        if (res.videoUrls && res.videoUrls.length > 0) {
            this.bounty.controls['videoUrls'].patchValue(res.videoUrls);
            this.urlForVideo = res.videoUrls;
        }
        if (res.imageUrls && res.imageUrls.length > 0) {
            this.bounty.controls['imageUrls'].patchValue(res.imageUrls);
            this.urlForImages = res.imageUrls;
        }

        // Currency, Amount, Cancellation Policy
        if (res.price === 0) {
            this.freeBounty = true;
            this.bounty.controls.price.patchValue(0);
        } else {
            this.bounty.controls.price.patchValue(res.price);
        }
        if (res.currency) { this.bounty.controls.currency.patchValue(res.currency); }
        if (res.cancellationPolicy) { this.bounty.controls.cancellationPolicy.setValue(res.cancellationPolicy); }

        // Status
        this.bounty.controls.status.setValue(res.status);

        // Gyan
        this.bounty.controls['academicGyan'].patchValue(res.academicGyan);

        this.bounty.controls['nonAcademicGyan'].patchValue(res.nonAcademicGyan);
    ...
    public bountyStepUpdate() {
        if (this.bounty.value.stage < this.step) {
            this.bounty.patchValue({
                'stage': this.step
            });
        }
    ...
    public addImageUrl(value: String) {
        ...
        const control = <FormArray>this.bounty.controls['imageUrls'];
        this.bountyImage1Pending = false;
        control.patchValue(this.urlForImages);
        const tempBountyData = this.bountyData;
        tempBountyData.imageUrls = this.bounty.controls['imageUrls'].value;
        this.sidebarMenuItems = this._leftSideBarService.updateBountySideMenu(tempBountyData, this.sidebarMenuItems);
    ...
    public addVideoUrl(value: String) {
        ...
        const control = this.bounty.controls['videoUrls'];
        this.bountyVideoPending = false;
        control.patchValue(this.urlForVideo);
        const tempBountyData = this.bountyData;
        tempBountyData.videoUrls = this.bounty.controls['videoUrls'].value;
        this.sidebarMenuItems = this._leftSideBarService.updateBountySideMenu(tempBountyData, this.sidebarMenuItems);
    }
    ...
    private checkStatusAndSubmit(data, timeline?, step?) {
        if (this.bounty.controls.status.value === 'active') {
            this.dialogsService.openCollectionCloneDialog({ type: 'bounty' })
                .subscribe((result) => {
                    if (result === 'accept') {
                        this.executeSubmitBounty(data, timeline, this.step);
    ...
    private executeSubmitBounty(data, timeline?, step?) {
        const lang = <FormArray>this.bounty.controls.language;
        lang.removeAt(0);
        lang.push(this._fb.control(data.value.selectedLanguage));
        const body = data.value;
        delete body.selectedLanguage;

        this._collectionService.patchCollection(this.bountyId, body).subscribe(
            (response: any) => {
                const result = response;
                let collectionId;
                if (result.isNewInstance) {
                    this.bounty.controls.status.setValue(result.status);
                    collectionId = result.id;
                } else {
                    collectionId = this.bountyId;
                }
                result.topics = this.bountyData.topics;
                result.contents = this.bountyData.contents;
                result.owners = this.bountyData.owners;
                this.sidebarMenuItems = this._leftSideBarService.updateBountySideMenu(result, this.sidebarMenuItems);
                ...
                if (step && step === this.timelineStep) {
                    ...
                    if (this.exitAfterSave) {
                        ...
                        this.step++;
                        this.bountyStepUpdate();
                        this.router.navigate(['bounty', collectionId, 'edit', this.step]);
                        this.busySavingData = false;
                    }
                }
        ...
        if (body.startDate && body.endDate && itinerary && itinerary.length > 0) {
            ...
            if (!itinerary || itinerary.length === 0) {
                if (this.exitAfterSave) {
                    ...
                    this.snackBar.open('You need to add at least 1 activity to your bounty to proceed.', 'Close', {
                        duration: 5000
                    });
                }
            } else {
                this.snackBar.open('No dates have been selected for your bounty.', 'Close', {
                    duration: 5000
                });
            }
        }
        ...
        if (topicArray.length !== 0) {
            if (this.originalInterests.length > 0) {
                ...
                this.originalInterests.forEach(interest => {
                    ...
                    unlinkObeservables.push(this._collectionService.unlinkTopic(this.bountyId, interest.id));
                });
                console.log(unlinkObeservables);
                const finalObservable = merge(...unlinkObeservables);
                finalObservable.pipe(
                    flatMap(res => {
                        return this.http.patch(environment.apiUrl + '/api/collections/' + this.bountyId + '/topics/rel', body, this.requestHeaderService.options);
                    })
                ).subscribe((res: any) => {
                    this._collectionService.getCollectionDetail(this.bountyId, this.query)
                        .subscribe((resData) => {
                            this.sidebarMenuItems = this._leftSideBarService.updateBountySideMenu(resData, this.sidebarMenuItems);
                        });
                    this.busySavingData = false;
                    if (this.exitAfterSave) {
                        this.exit();
                ...
                this.http.patch(environment.apiUrl + '/api/collections/' + this.bountyId + '/topics/rel', body, this.requestHeaderService.options)
                    .subscribe((res: any) => {
                        this._collectionService.getCollectionDetail(this.bountyId, this.query)
                            .subscribe((resData) => {
                                this.sidebarMenuItems = this._leftSideBarService.updateBountySideMenu(resData, this.sidebarMenuItems);
    ...
    public goto(toggleStep) {
        ...
        this.router.navigate(['bounty', this.bountyId, 'edit', +toggleStep]);
        this.showBackground = this.step && this.step.toString() === '6';
    }
    ...
    submitForReview() {
        // Post Bounty for review
        this._collectionService.submitForReview(this.bountyId)
            .subscribe((res: any) => {
                this.bounty.controls.status.setValue('submitted');
                console.log('Bounty submitted for review');
                this.isSubmitted = true;
                this.dialogsService.openCollectionSubmitDialog({ type: 'bounty' });
                // call to get status of bounty
                if (this.bounty.controls.status.value === 'active') {
                    this.sidebarMenuItems[3].visible = false;
                    this.sidebarMenuItems[4].visible = true;
                    this.sidebarMenuItems[4].active = true;
        ...
        this.dialogsService
            ...
            .subscribe((res: any) => {
                if (res) {
                    ...
                    this.bounty.controls.selectedLanguage.patchValue(res.name);
                }
            });
    ...
    private updatePayoutRule(newPayoutId) {
        if (this.payoutRuleNodeId) {
            ...
            this._paymentService.postPayoutRule(this.bountyId, newPayoutId).subscribe((res: any) => {
                if (res) {
                    this.payoutLoading = false;
                    this.payoutRuleAccountId = newPayoutId;
    ...
    onFreeChange(event) {
        if (event) {
            this.bounty.controls['price'].setValue(0);
        }
    }
    ...
    public submitAssessment() {
        ...
        this.busySavingData = true;
        this._collectionService.updateAssessmentModel(this.bountyId, {
    ...
    public importProfileBio() {
        this.bounty.controls.aboutHost.patchValue(this.bountyData.owners[0].profiles[0].description);
    }
