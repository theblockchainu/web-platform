	...
	public resetpwd() {
		...
		this.authenticationService.resetPassword(body)
			.subscribe(
				(data) => {
					if (data.success) {
						this.snackBar.open('Password reset successfully. Redirecting to login...', '', {
							duration: 5000
						});
				...
				(error) => {
					console.log(error);
					this.snackBar.open('This reset link has expired.', 'Email new link?').onAction().subscribe(res => {
