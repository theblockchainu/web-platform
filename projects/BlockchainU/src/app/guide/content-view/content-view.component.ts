...
export class ContentViewComponent implements OnInit, AfterViewInit {
	...
	public findAndOpenDialog(index) {
		...
		switch (contentType) {
			case 'in-person':
				dialogRef = this.dialog.open(GuideContentInpersonComponent, { panelClass: 'responsive-dialog', data: { itenaryForm: this.itenaryForm, index: index, isEdit: isEdit }, disableClose: true, hasBackdrop: true, width: '45vw', height: '100vh' });
				break;
			case 'project':
				dialogRef = this.dialog.open(GuideContentProjectComponent, { panelClass: 'responsive-dialog', data: { itenaryForm: this.itenaryForm, index: index, isEdit: isEdit, collectionStartDate: this.collectionStartDate, collectionEndDate: this.collectionEndDate }, disableClose: true, hasBackdrop: true, width: '45vw', height: '100vh' });
				break;
			case 'video':
				dialogRef = this.dialog.open(GuideContentVideoComponent, { panelClass: 'responsive-dialog', data: { itenaryForm: this.itenaryForm, index: index, isEdit: isEdit }, disableClose: true, hasBackdrop: true, width: '45vw', height: '100vh' });
				break;
			default:
				break;
		}
