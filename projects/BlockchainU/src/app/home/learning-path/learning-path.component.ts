...
export class LearningPathComponent implements OnInit {
	...
	public userId;
	public learningPaths: Array<any>;
	public learningPathsBackup: Array<any>;
	...
	private setTags() {
		this.titleService.setTitle('LearningPaths');
		this.metaService.updateTag({
			property: 'og:title',
			content: 'Explore LearningPaths'
		});
		this.metaService.updateTag({
	...
	private fitlerResults() {
		// this.learningPaths = this.learningPathsBackup.filter((val) => {
				...
				'include': [
					{
						...
						'scope': {
							'where': { 'type': 'learningPath', 'status': 'active' }
						}
					}
				]
			};
				...
				(response: any) => {
					...
					for (const responseObj of response) {
						responseObj.collections.forEach(collection => {
							// let learningPathLocation = 'Unknown location';
							// let lat = 37.5293864;
							// let lng = -122.008471;
							if (collection.status === 'active') {
								...
								// 			learningPathLocation = content.locations[0].city;
								// 			lat = parseFloat(content.locations[0].map_lat);
								// 			lng = parseFloat(content.locations[0].map_lng);
								// 		}
								// 	});
								// 	collection.location = learningPathLocation;
								// 	collection.lat = lat;
								// 	collection.lng = lng;
								// }
					...
					this.learningPaths = _.uniqBy(learningPaths, 'id');
					this.learningPaths = _.orderBy(this.learningPaths, ['createdAt'], ['desc']);
					this.learningPathsBackup = _.cloneDeep(this.learningPaths);
	...
	private setFilterData() {
		...
		this.learningPaths.forEach(learningPath => {
			if (maxPrice < learningPath.price) {
				maxPrice = learningPath.price;
			}
			if (maxDuration < learningPath.totalHours) {
				maxDuration = learningPath.totalHours;
			}

			learningPath.language.forEach(language => {
				if (!this.languageList.includes(language)) {
					this.languageList.push(language);
				}
			});

			if (!this.locationList.includes(learningPath.location)) {
				this.locationList.push(learningPath.location);
			}

			if (!this.levelList.includes(learningPath.difficultyLevel)) {
				this.levelList.push(learningPath.difficultyLevel);
			}
	...
	public onLearningPathRefresh(event) {
		if (event) {
			this.fetchLearningPaths();
		}
	}
